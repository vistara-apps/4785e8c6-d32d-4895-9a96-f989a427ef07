  ...
  const actions = [
    {
      icon: Plus,
      title: 'Add Token',
      description: 'Track new holdings',
      price: 'Free',
      color: 'from-accent to-yellow-500',
      onClick: () => {
        // Scroll to token holdings section
        const tokenSection = document.querySelector('[data-testid="token-holdings"]');
        if (tokenSection) {
          tokenSection.scrollIntoView({ behavior: 'smooth' });
          // Trigger add token form after scroll
          setTimeout(() => {
            const addButton = document.querySelector('[data-testid="add-token-button"]') as HTMLButtonElement;
            if (addButton) {
              addButton.click();
            }
          }, 500);
